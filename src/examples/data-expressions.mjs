...
const dx = grammar`
  ...
  Pair      = Key "?"? ":" AltExpr  : ${(key, isOptional, _, expr) => ({ key, isOptional, expr })}
  AltExpr   = PathExpr ++ "|"       : ${(expr) => alt(expr)}
  PathExpr  = Expr ++ line?         : ${(exprs) => seq(exprs)}
  Expr      = "." Key "?"?          : ${(_, k, isOptional) => key(k, isOptional)}
...
const key = (key, optional = false) => function * (focus) {
...
const alt = (patterns) => function * (focus) {
