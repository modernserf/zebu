...
class InvalidBracketLiteralError extends Error {
  constructor(bracket) {
    super(
      `"${bracket}" is reserved for brackets, and cannot be used as a keyword or operator`
    );
  }
}
...
const sepByWithLines = (expr, sep) =>
  alt(sepBy(expr, seq(id, alt(sep, seq(_2, optLine, sep)), optLine)));
...
const compiler = createCompiler({
  ...
  sepBy: (expr, sep, ctx) => sepByWithLines(ctx.eval(expr), ctx.eval(sep)),
  ...
  wrappedParen: (content, ctx) =>
    wrappedWith("(", () => wrapIgnoreLines(ctx.eval(content)), ")"),
  ...
  literal: (value) => {
    ...
    if (/[(){}[\]]/.test(value)) {
      throw new InvalidBracketLiteralError(value);
    }
    return lit(value);
  },
});
